...
class ResNet(nn.Module):
    ...
    def __init__(self, num_classes: int, width_mult: int = 1, input_channels: int = 3):
        """Initialize ResNet.
...
def create_substitute(
    ...
    """
    if arch == "resnet18":
        return ResNet(num_classes=num_classes, width_mult=width_mult, input_channels=input_channels)
    elif arch == "resnet18-8x":
        return ResNet(num_classes=num_classes, width_mult=8, input_channels=input_channels)
    elif arch == "lenet":
        return LeNet(num_classes=num_classes, input_channels=input_channels)
    elif arch == "lenet_mnist":
        return LeNetMNIST(num_classes=num_classes, input_channels=input_channels)
    else:
